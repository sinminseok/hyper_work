...
@Service
public class GroundContactTimeRankService extends AbstractGameRankService {

    private final GameRepository gameRepository;
    private final GameHistoryRepository gameHistoryRepository;
    private final UserRepository userRepository;

    public GroundContactTimeRankService(GameHistoryRepository gameHistoryRepository, UserRepository userRepository, GameRepository gameRepository, GameHistoryRepository gameHistoryRepository1, UserRepository userRepository1) {
    ...
    protected List<GameHistory> fetchSortedHistories(Game game) {
        ...
        histories.sort((g1, g2) -> {
            ...
            if (!g1.isDone() && g2.isDone()) return 1;
            return Double.compare(g1.getCurrentGroundContactTime(), g2.getCurrentGroundContactTime());
        });
        return histories;
    }
    ...
    public void generateGame(LocalDate date) {
        for (GameDistance distance : GameDistance.values()) {
            for (int i = 5; i <= 23; i += distance.getTime()) {
                ...
                Game game = createGame(GameType.GROUND_CONTACT_TIME, distance, date, i);
    ...
    public GameType getGameType() {
        return GameType.GROUND_CONTACT_TIME;
    }
}
