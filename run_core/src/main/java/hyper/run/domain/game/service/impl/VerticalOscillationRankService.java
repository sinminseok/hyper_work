...
public class VerticalOscillationRankService extends AbstractGameRankService {
    ...
    protected List<GameHistory> fetchSortedHistories(Game game) {
        ...
        histories.sort((g1, g2) -> {
            ...
            if (!g1.isDone() && g2.isDone()) return 1;
            return Double.compare(g1.getCurrentVerticalOscillation(), g2.getCurrentVerticalOscillation());
        });
    ...
    public void generateGame(LocalDate date) {
        for (GameDistance distance : GameDistance.values()) {
            for (int i = 5; i <= 23; i += distance.getTime()) {
                ...
                Game game = createGame(GameType.VERTICAL_OSCILLATION, distance, date, i);
    ...
    public GameType getGameType() {
        return GameType.VERTICAL_OSCILLATION;
    }
}
